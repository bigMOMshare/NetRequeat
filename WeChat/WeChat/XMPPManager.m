...
- (void)setUpXMPP{
    ...
    //2.配置服务器
    //(1)服务器IP地址
    [_stream setHostName:@"218.241.181.202"];
    //(2)配置服务器端口
...
//      message.from 消息来源
    ...
    if ([message isChatMessageWithBody]) {
        ...
        NSDictionary *dic = @{@"body" : message.body,@"from" : message.from};
        
        //发送通知,将消息内容和消息来源传递过去
        [[NSNotificationCenter defaultCenter]postNotificationName:@"kDidReceiveMessage" object:nil userInfo:dic];
    }
